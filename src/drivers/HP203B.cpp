...
{
    wire_.beginTransmission(ADDRESS);
    wire_.write(CMD_READ_PT);

    if (wire_.endTransmission(false) != 0)
        return false;

    if (wire_.requestFrom((uint8_t)ADDRESS, (uint8_t)6) != 6 ||
        wire_.available() < 6)
    {
    ...
    // raw[0..2] = Temperature
    // raw[3..5] = Pressure
    const float temperature = signed24(raw) / 100.0f;
    const float pressure = signed24(raw + 3) / 100.0f;

    if (pressure <= 0.0f || seaLevelHpa <= 0.0f)
    {
        return false;
    }
    ...
    data.altitudeM =
        44330.0f *
        (1.0f - powf(pressure / seaLevelHpa, 0.19029495f));
