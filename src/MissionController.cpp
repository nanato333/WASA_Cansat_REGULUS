...
bool MissionController::handleCommand(uint8_t c,uint32_t now,bool hasGoal,double lat,double lon){
 ...
 if(cmd==MissionCommand::CONFIRM_DEPLOYED){if(mode_!=OperationMode::MISSION||state_!=MissionState::LAUNCH)return false;transitionTo(MissionState::DEPLOYED,now);return true;}
 if(cmd==MissionCommand::CONFIRM_LANDED){bool flightRecovery=mode_==OperationMode::MISSION&&(state_==MissionState::LAUNCH||state_==MissionState::DEPLOYED);bool groundTest=mode_==OperationMode::DEVELOPMENT&&state_==MissionState::BOOT;if(!flightRecovery&&!groundTest)return false;transitionTo(MissionState::LANDED,now);return true;}
