...
void setup() {
    ...
    if (!imu.begin()) Serial.println("QMI8658 initialization failed (try address 0x6A if SA0 is low)");
    else Serial.printf("QMI8658 initialized at 0x%02X\n", imu.address());
}
void loop() {
    static uint32_t previous = 0; const uint32_t now = millis();
    if (now - previous < 100) return; previous = now;
    QMI8658::Data d;
    if (!imu.read(d)) { Serial.println("QMI8658 read failed"); return; }
